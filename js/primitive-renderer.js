...
var htmlRenderInto = function(t, e) {
  ...
  tplHtmlTable = function(t) {
    ...
    for (
      i.push("<thead>"),
        ...
        t.range === RANGE_TODAY
          ...
          : t.range === RANGE_AVERAGE
            ? i.push(
                "        Daily averages since " +
                  t.dateStart +
                  " (" +
                  t.daysSinceStart +
                  " day" +
                  (t.daysSinceStart > 1 ? "s" : "") +
                  ")"
              )
            : t.range === RANGE_ALLTIME &&
              i.push(
                "        Aggregate data since " +
                  t.dateStart +
                  " (" +
                  t.daysSinceStart +
                  " day" +
      ...
      (r = t.domains[e]),
        ...
        i.push(
          '    <tr class="domain" data-connect-id="' +
            a +
            '" data-url="' +
            r.name +
        ...
        i.push("        </td>"),
        i.push('        <td class="time">'),
        "h" !== t.resolution &&
          i.push(
            '            <span class="time-day">' +
              tplHtmlTableTimeFragment(n.day) +
              "</span>"
          ),
        i.push(
          '            <span class="time-hour">' +
            tplHtmlTableTimeFragment(n.hour) +
            "</span>"
        ),
        i.push(
          '            <span class="time-minute">' +
            tplHtmlTableTimeFragment(n.minute) +
            "</span>"
        ),
        i.push(
          '            <span class="time-second">' +
            tplHtmlTableTimeFragment(n.second) +
            "</span>"
        ),
        i.push("        </td>"),
        i.push("    </tr>");
    return (
      ...
      i.push("<tfoot>"),
      t.domains.length > 0
        ? ((n = helperGetTimeObj(t.total.time, t.resolution)),
  ...
  tplHtmlTableTimeFragment = function(t) {
    ...
    return (
      e.push(
        '<span class="digit-tens ' +
          (t.implicitlyActive || t.tens > 0 ? "active" : "") +
          '">'
      ),
      e.push("" + t.tens),
      e.push("</span>"),
      e.push(
        '<span class="digit-units ' +
          (t.implicitlyActive || t.tens > 0 || t.units > 0 ? "active" : "") +
          '">'
      ),
      e.push("" + t.units),
      e.push(t.unit),
  ...
  tplHtmlTimeObjectFragment = function(t) {
    ...
    return (
      "h" !== t.resolution &&
        e.push(
          '<span class="time-day">' +
            tplHtmlTableTimeFragment(r.day) +
            "</span>"
        ),
      e.push(
        '<span class="time-hour">' +
          tplHtmlTableTimeFragment(r.hour) +
          "</span>"
      ),
      e.push(
        '<span class="time-minute">' +
          tplHtmlTableTimeFragment(r.minute) +
          "</span>"
      ),
      e.push(
        '<span class="time-second">' +
          tplHtmlTableTimeFragment(r.second) +
          "</span>"
      ),
      e.join("")
    );
  },
  tplHtmlIdleTimeControl = function(t) {
    var e = [];
    return (
      e.push(
        '<input tabindex="-1" class="slider" type="range" min="' +
          t.min +
          '" max="' +
          t.max +
          '" value="' +
      ...
      e.push("<br/>"),
      e.push(
        'Stop tracking if no <span title="Mouse or keyboard">activity</span> detected for <span class="display">' +
          t.computed +
          "</span>"
      ),
      e.join("")
  ...
  tplHtmlGraphGapControl = function(t) {
    var e = [];
    return (
      e.push(
        '<input tabindex="-1" class="slider" type="range" min="' +
      ...
      e.push("<br/>"),
      e.push(
        'Gap between graph parts: <span class="display">' + t.raw + "</span>"
      ),
      e.join("")
    );
  },
  tplHtmlBadgeDisplayControl = function(t) {
    ...
    return (
      e.push("<label>"),
      e.push(
        '<input tabindex="-1" class="checkbox" type="checkbox" ' +
          (t.checked ? "checked" : "") +
          " />"
      ),
      e.push("Display time tracker in icon"),
      e.push("</label>"),
      e.join("")
    );
  },
  tplElementStatsDomain = function(t, e) {
    ...
    return (
      a.querySelector(".url").setAttribute("href", "http://" + t.url),
      (a.querySelector(".url").textContent = t.url),
      (a.querySelector(".rank-" + RANGE_TODAY + "-position").textContent =
        t.ranks[RANGE_TODAY].position),
      (a.querySelector(".rank-" + RANGE_TODAY + "-total").textContent =
        t.ranks[RANGE_TODAY].total),
      (a.querySelector(".rank-" + RANGE_ALLTIME + "-position").textContent =
        t.ranks[RANGE_ALLTIME].position),
      (a.querySelector(".rank-" + RANGE_ALLTIME + "-total").textContent =
        t.ranks[RANGE_ALLTIME].total),
      ...
      (a.querySelector(
        ".time-" + RANGE_TODAY
      ).innerHTML = tplHtmlTimeObjectFragment({
  ...
  tplElementStatsOverall = function(t, e) {
    return (
      (e.querySelector(".days-visited").textContent = t.days.visited),
  ...
  tplElementStatsCharts = function(t, e) {
    ...
    return (
      (r.querySelector(".info .left strong").textContent = t.dates.start),
      (r.querySelector(".info .right strong").textContent = t.dates.today),
      e
    );
  },
  tplElementDoughnut = function(t, e) {
    var r,
      ...
      d = l / 2 - p / 2,
      h = 0,
      g = { name: "other", percentage: 0, color: GRAPH_COLOR_OTHER },
      A = !0,
      b = "http://www.w3.org/2000/svg",
      S = document.createElementNS(b, "svg:svg");
    for (
      S.setAttribute("xmlns:wt", "http://www.example.com/webtime-tracker"),
      ...
      (A = !1),
        (r = t.domains[u]),
        r.graphed
          ? (tplElementDoughnutWedgeFragment(r, {
              percentageStart: h,
              noData: A,
              graphGap: e,
              graphSize: l,
            ...
            (h += r.percentage))
          : (g.percentage += r.percentage);
    return (
      g.percentage > 0 &&
        ((g.percentageString = getPercentageString(g.percentage)),
        tplElementDoughnutWedgeFragment(g, {
          percentageStart: h,
          noData: A,
          graphGap: e,
          graphSize: l,
      ...
      A &&
        ((g.name = ""),
        (g.percentage = 100),
        (g.percentageString = "No data available"),
        tplElementDoughnutWedgeFragment(g, {
          percentageStart: h,
          noData: A,
          graphGap: e,
      ...
      i.setAttribute("class", "percentage"),
      A && (i.innerHTML = "No data available"),
      s.appendChild(i),
      (i = document.createElement("div")),
  ...
  tplElementDoughnutWedgeFragment = function(t, e) {
    ...
    if (
      ((i = S * e.percentageStart),
      (u = S * (e.percentageStart + t.percentage - R)),
      (a = document.createElementNS(b, "circle")),
      e.noData)
    )
      (r = document.createElementNS(b, "circle")),
        r.setAttribute("cx", y),
        r.setAttribute("cy", E),
        r.setAttribute("r", O),
        (n = document.createElementNS(b, "circle"));
    else if (0 === i && 100 === t.percentage)
      (r = document.createElementNS(b, "circle")),
        r.setAttribute("cx", y),
        r.setAttribute("cy", E),
        r.setAttribute("r", T),
    ...
    return (
      ...
      s.setAttribute("wt:percentage-string", t.percentageString),
  ...
  tplElementChartStatsDays = function(t) {
    ...
    for (e = 0; e < a.length; e++)
      a[e].height = (a[e].seconds / t.timeValueMax) * n;
    var s,
      i,
      u,
      l,
    ...
    var p = r / t.daysTotal;
    for (e = 0; e < a.length; e++)
      (s = Math.max(t.stepHeightMin, a[e].height)),
        (i = document.createElementNS(c, "rect")),
  ...
  tplElementChartStatsDaynames = function(t) {
    var e,
      ...
      i = [];
    for (e = 0; e < t.daynames.length; e++)
      i[e] = { seconds: t.daynames[e], height: (t.daynames[e] / a) * n };
    var u,
      l,
      c,
      o,
